...
#system files
system = ['dateScript.py','template.html','requirements.txt', 'Pipfile', 'Pipfile.lock', '.venv','Attendance_generator','Input','Output']
...
#chaning into the Input folder
root = f'{cwd}/input'
os.chdir(root)

# using the os moduile to create a list of all files in the root directory
...
for folder in directory:
    ...
    if (folder in system) or ('_attendance_5_2023.pdf' in folder) or ('.zip' in folder):
        ...
        #events are filted to exclude any date greater than todays date
        ...
        #we change dirctory into the parent folder 
        os.chdir(f'..')

        #folder is removed from file once completed
        shutil.rmtree(folder)

        #we change dirctory into the output folder
        os.chdir(f'{cwd}/output')
